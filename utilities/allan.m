function [retval, s, errorb, tau] = allan(data,tau,name,verbose,imu_axis)
...
if verbose >= 2, fprintf(1,'allan: %s\n\n',versionstr); end
...
if ~isfield(data,'freq')
    ...
    if verbose >= 2, fprintf(1,'allan: Fractional frequency data generated from phase data (M=%g).\n',length(data.freq)); end
...
if isfield(data,'rate') && data.rate > 0 % if data rate was given
    if verbose >= 2, fprintf(1, 'allan: regular data (%g data points @ %g Hz)\n',length(data.freq),data.rate); end
    ...
    if verbose >= 2, fprintf(1,'allan: calculating Allan deviation...\n       '); end
...
if verbose >= 2 % show all data
    ...
    try
        ...
        if verbose >= 2, fprintf(1,'allan: Note: Install dsplot.m for improved plotting of large data sets (File Exchange File ID: #15850).\n'); end
...
if verbose >= 1 % show ADEV results
    ...
    if ~isempty(sm)
        axis_color = '.-b';
        if (imu_axis == 2), axis_color = '.-g'; end;
        if (imu_axis == 3), axis_color = '.-m'; end;
        
        % Choose loglog or semilogx plot here    #PLOTLOG
        %semilogx(tau,sm,'.-b','LineWidth',plotlinewidth,'MarkerSize',24);
        loglog(tau, sm, axis_color, 'LineWidth', plotlinewidth, 'MarkerSize', 24);
